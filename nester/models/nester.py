...
class Nester(models.AbstractModel):
    ...
    def nest(self, shapes, sheets):
        ...
        while shapes:
            ...
            selected_sheets = self.select_sheets(selected_shape, sheets)

            for sheet in selected_sheets:
                sheet.update({
                    'remaining_qty': sheet.get('remaining_qty', 0) - sheet.get('required_qty', 0)
                })



            self.print_selected_sheets(selected_shape, selected_sheets)

            selected_shape.update({
                'remaining_qty': 0
            })

            if selected_shape.get('remaining_qty', 0) <= 0:
                # Remove the shape from the list if fully nested
                shapes.pop(0)
